...
if [ "$BASH" != "/bin/bash" ]; then
  echo ""
  echo "Please use bash to run this script, like so: bash Blockchain_Testing.sh"
  echo ""
  exit
fi
...
if [ ! -f BlockchainMultiThreaded.java ]; then
	...
	echo ""
	exit
elif [ ! -d multi_blockchain/src ]; then
	echo ""
	echo " Error: You must place the multi_blockchain/src folder in this directory"
	echo "        before we can proceed. Aborting test script."
	echo ""
	exit
elif [ ! -d single_blockchain/src ]; then
	echo ""
	echo " Error: You must place the multi_blockchain/src folder in this directory"
	echo "        before we can proceed. Aborting test script."
	echo ""
	exit
elif [ ! -f single_blockchain/src/main.rs ]; then
	echo ""
	echo " Error: You must place main.rs in the single_blockchain/src directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
elif [ ! -f multi_blockchain/src/main.rs ]; then
	echo ""
	echo " Error: You must place main.rs in the multi_blockchain/src directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
...
################################################################################
# Clear any old text from output text files (to avoid mixmatched data)
################################################################################

> Data_Output/JAVA_SingleThreaded_Data.txt
> Data_Output/JAVA_MultiThreaded_Data.txt
> Data_Output/CPP_SingleThreaded_Data.txt
> Data_Output/CPP_MultiThreaded_Data.txt
> Data_Output/RUST_SingleThreaded_Data.txt
> Data_Output/RUST_MultiThreaded_Data.txt
...
do
	...
	do
		...
		java BlockchainSingleThreaded <Data_Input/TestCase$i.txt $j >> Data_Output/JAVA_SingleThreaded_Data.txt 2> /dev/null
		execution_val=$?
		...
		java BlockchainMultiThreaded <Data_Input/TestCase$i.txt $j >> Data_Output/JAVA_MultiThreaded_Data.txt 2> /dev/null
		execution_val=$?
		if [[ $execution_val != 0 ]]; then
...
#Attempt to compile multi-threaded blockchain.
g++ -o blockchainMultiThreaded blockchainMultiThreaded.cpp sha256.cpp -pthread 2> /dev/null
compile_val=$?
if [[ $compile_val != 0 ]]; then
	echo "fail multi-threaded (failed to compile)"
fi
...
do
	...
	do
		...
		# Run single-threaded program. Capture return value to check whether it crashes.
		./blockchainSingleThreaded ./Data_Input/TestCase$i.txt $j >> Data_Output/CPP_SingleThreaded_Data.txt 2> /dev/null
		execution_val=$?
		if [[ $execution_val != 0 ]]; then
			echo "fail single-threaded (program crashed)"
			continue
		...
		# Run multi-threaded program. Capture return value to check whether it crashes.
		./blockchainMultiThreaded ./Data_Input/TestCase$i.txt $j >> Data_Output/CPP_MultiThreaded_Data.txt 2> /dev/null
		execution_val=$?
		if [[ $execution_val != 0 ]]; then
			echo "fail multi-threaded (program crashed)"
			continue
...
# Attempt to compile single-threaded blockchain.
(cd single_blockchain/src && cargo build) 2> /dev/null
compile_val=$?
if [[ $compile_val != 0 ]]; then
	echo "fail single-threaded (failed to compile)"
fi
...
#Attempt to compile multi-threaded blockchain.
(cd multi_blockchain/src && cargo build) 2> /dev/null
compile_val=$?
if [[ $compile_val != 0 ]]; then
	echo "fail multi-threaded (failed to compile)"
fi
...
do
	...
	do
		...
		# Run single-threaded program. Capture return value to check whether it crashes.
		(cd single_blockchain/src && cargo run $j ../Data_Input/TestCase$i.txt) >> Data_Output/RUST_SingleThreaded_Data.txt 2> /dev/null
		execution_val=$?
		if [[ $execution_val != 0 ]]; then
			echo "fail single-threaded (program crashed)"
			continue
		...
		# Run multi-threaded program. Capture return value to check whether it crashes.
		(cd multi_blockchain/src && cargo run $j ../Data_Input/TestCase$i.txt) >> Data_Output/RUST_MultiThreaded_Data.txt 2> /dev/null
		execution_val=$?
		if [[ $execution_val != 0 ]]; then
			echo "fail multi-threaded (program crashed)"
			continue
